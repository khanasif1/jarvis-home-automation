...
`azure-backend`. The compiled ARM JSON stays under the repository's disposable
...
`azd` discovers `infra/main.bicep` via the repository's root `azure.yaml`
(owned outside this folder). From the repository root:
...
The `infrastructure-validate.yml` workflow (see `.github/workflows/`) is
expected to run, at minimum:
...
against every pull request that touches `infra/**`, without building,
testing, or packaging `pi-client/**` or `azure-backend/**`.
