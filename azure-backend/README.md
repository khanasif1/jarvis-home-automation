...
azure-backend/
  ...
  requirements-dev.txt         # + pytest/pytest-cov, for local dev and CI
  pytest.ini                   # pytest config: src on path, artifacts under repo .test-artifacts/
  prompts/
...
From the repository root (matches CI exactly):
...
works from the repo root without manually exporting `PYTHONPATH`. All
generated pytest cache/artifacts are written under the repository-root
...
`azd deploy azure-backend` (see `azure.yaml`, `.github/workflows/azure-backend-deploy.yml`)
packages this directory as-is except for the paths listed in `.funcignore`
