...
etc.) should be written under `<repo-root>/.test-artifacts/`, never
...
```bash
pi-client/packaging/build-release.sh            # Linux/macOS/CI
...
`<repo-root>/.test-artifacts/pi-client-release/dist/`; pass
`--output-dir`/`-OutputDir` to write elsewhere (e.g. from CI).
