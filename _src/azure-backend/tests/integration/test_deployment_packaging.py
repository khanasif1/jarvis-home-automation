...
2. The deployment package (whatever ``.funcignore`` allows through) must
   ...
   ``pi-client``/``infra``. We prove this by copying the backend into a
   scratch directory under source-root ``.test-artifacts/``, applying the same
...
_SOURCE_ROOT = _BACKEND_ROOT.parent
_ARTIFACTS_ROOT = _SOURCE_ROOT / ".test-artifacts" / "azure-backend-deployment-package"
